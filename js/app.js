...
class LifeCounterApp {
  ...
  constructor() {
    ...
    this.nameInputs = Array.from(document.querySelectorAll('#name-input input'));
  ...
  migrateLegacyState() {
    ...
    state.iconDisplay = localStorage.getItem('iconDisp') || defaults.iconDisplay;
    state.playerLayout = localStorage.getItem('playerNumber') || defaults.playerLayout;
    const storedInitLife = parseInt(localStorage.getItem('initLife'), 10);
    state.initialLife = Number.isFinite(storedInitLife) ? storedInitLife : defaults.initialLife;
    state.poisonDisplay = localStorage.getItem('poisonDisp') || defaults.poisonDisplay;
    ...
    for (let index = 0; index < MAX_PLAYERS; index += 1) {
      ...
      player.life = Number.isFinite(storedLife) ? storedLife : state.initialLife;
      player.poison = Number.isFinite(storedPoison) ? Math.max(0, storedPoison) : 0;
      if (storedName) {
  ...
  mergeWithDefaults(partial) {
    ...
    state.iconDisplay = typeof partial.iconDisplay === 'string' ? partial.iconDisplay : defaults.iconDisplay;
    state.playerLayout = typeof partial.playerLayout === 'string' ? partial.playerLayout : defaults.playerLayout;
    state.initialLife = Number.isFinite(partial.initialLife) ? Math.trunc(partial.initialLife) : defaults.initialLife;
    state.poisonDisplay = typeof partial.poisonDisplay === 'string' ? partial.poisonDisplay : defaults.poisonDisplay;
    ...
    for (let index = 0; index < MAX_PLAYERS; index += 1) {
      ...
      const life = Number.isFinite(incoming.life) ? Math.trunc(incoming.life) : fallback.life;
      const poison = Number.isFinite(incoming.poison) ? Math.max(0, Math.trunc(incoming.poison)) : fallback.poison;
      const rawName = typeof incoming.rawName === 'string' ? incoming.rawName : fallback.rawName;
      ...
      state.players[index] = {
        ...
        poison,
        rawName,
        name: typeof incoming.name === 'string' && incoming.name.trim() ? incoming.name : displayName,
      };
    }
  ...
  registerEventListeners() {
    ...
    if (this.reloadButton) {
      this.reloadButton.addEventListener('click', () => {
        this.withTemporaryClass(this.reloadButton.querySelector('i'), 'fa-spin-fast', 500);
        this.resetLife();
      });
    }
    ...
    if (this.resetSettingsButton) {
      this.resetSettingsButton.addEventListener('click', () => {
        this.resetSettings();
        this.withTemporaryClass(this.resetSettingsButton.querySelector('i'), 'fa-jump', 1000);
      });
    }

    this.nameInputs.forEach((input, index) => {
      input.addEventListener('input', () => this.updatePlayerName(index, input.value));
    });
    ...
    window.addEventListener('pointerdown', (event) => {
      ...
      this.swipeStart = inSettings ? null : { x: event.clientX, y: event.clientY };
    });
  ...
  handleSettingsButton(button) {
    ...
    if (button.dataset.iconDisp) {
      this.setIconDisplay(button.dataset.iconDisp);
    } else if (button.dataset.playerNumber) {
      this.setPlayerLayout(button.dataset.playerNumber);
    } else if (button.dataset.lifeAmount) {
      this.setInitialLife(parseInt(button.dataset.lifeAmount, 10));
    } else if (button.dataset.poisonCounter) {
      this.setPoisonDisplay(button.dataset.poisonCounter);
    } else if (button.dataset.setFullscreen) {
      this.setFullscreenPreference(button.dataset.setFullscreen);
    } else if (button.dataset.nameDisp) {
      this.setNameDisplay(button.dataset.nameDisp === '1');
    }
  }
  ...
  applyIconDisplay() {
    ...
    this.menuSettings.style.display = this.state.iconDisplay === 'none' ? 'none' : '';
  }
  ...
  applyPlayerLayout() {
    ...
    const classesToRemove = Array.from(this.mainElement.classList).filter((className) =>
      className.startsWith(layoutClassPrefix),
    );
    classesToRemove.forEach((className) => this.mainElement.classList.remove(className));
    this.mainElement.classList.add(`${layoutClassPrefix}${this.state.playerLayout}`);
  }
  ...
  updateSettingsUI() {
    this.setActiveSetting('icon-disp', `[data-icon_disp="${this.state.iconDisplay}"]`);
    this.setActiveSetting('player-number', `[data-player_number="${this.state.playerLayout}"]`);
    this.setActiveSetting('life-amount', `[data-life_amount="${this.state.initialLife}"]`);
    this.setActiveSetting('poison-counter', `[data-poison_counter="${this.state.poisonDisplay}"]`);
    this.setActiveSetting('set-fullscreen', `[data-set_fullscreen="${this.state.fullscreen ? 'set' : 'cancel'}"]`);
    this.setActiveSetting('name-disp', `[data-name_disp="${this.state.showNames ? '1' : '0'}"]`);

    if (this.menuSettings) {
      this.menuSettings.setAttribute('aria-expanded', this.settingsPanel.classList.contains('active') ? 'true' : 'false');
    }
  }
